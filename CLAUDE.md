...
- `src/components/block-browser.tsx` — Progress/Stages block picker (chips + search); `ref-photo-strip.tsx` — project reference thumbnails; `stage-row.tsx`, `progress-item-row.tsx`
...
- `supabase/migrations/` — 0001 init · 0002 projects+reports · 0003 soft-edit+no_work · 0004 grants · 0005 backdated · 0006 Phase 2 schema · 0007 delivery issues+`photos.delivery_id` · 0008 `photos.archived_at` · 0009 purchase requests · 0011 stock_counts · 0012 machinery-in-report · 0013 visitors · 0014 PR line items · 0015 PR member-update RLS · 0016 request photos · 0017 blocks+stages · 0018 progress tracker · 0019 project reference photos
...
## Migrations
**Applied through 0019 on hosted Supabase.** (0001–0008 earlier; 0009–0013 on 2026-05-27; 0014 + 0015 on 2026-05-28/29; 0016–0019 on 2026-05-29.) `0016` = `photos.purchase_request_id` (request photos). `0017` = `project_blocks` + `block_stages`. `0018` = `project_blocks.unit_count` + `block_progress_items` + `photos.progress_item_id`/`block_stage_id` (A–L progress tracker). `0019` = `photos.is_project_ref` (project reference photos).
**Apply-before-deploy rule:** any migration that adds a column the read layer filters on (e.g. `0019` → `getProjectPhotos` filters `is_project_ref`) must be applied **before** the code deploys, or the affected reads return empty / error. For those, commit + hold the push until Stanley confirms "applied", then push.
