...
- **Phase 5 (operations):** 🔧 Machinery daily log + weekly stock counts built & build-verified 2026-05-27 (**migration `0011` (stock) PENDING**; 0010 applied). Remaining: visitors/equipment on the daily report.
...
**Machinery daily log.** Machines are company-scoped reference data (managed in `/office/catalog`, "Machines" section, pm/office). Supervisor logs a machine per day at `/app/projects/[id]/machinery` (`machine-log-form.tsx`): machine + date + present / hours / breakdown(+note) / operator / fuel (per-day total +note). Upsert keyed `(project_id, machine_id, log_date)` so re-logging updates the day. Office sees a read-only recent-logs list on the project page. Data: `machinery.ts` (`getMachines`/`getProjectMachineLogs`). Actions: `createMachine`/`logMachine`. Migration `0010_machinery.sql` adds `machines` + `machine_logs` + RLS + grants. (Same client/server gotcha as taxonomy: client form imports only `type Machine`, inlines the label — never import a runtime value from a `server-only` module into a client component.)
...
## Migrations
**Applied through 0010 + storage_setup on hosted Supabase (0009 purchase-requests + 0010 machinery applied 2026-05-27). PENDING apply: `0011` (stock counts).**
