...
- **Phase 5 (operations):** 🔧 Machinery daily log built & build-verified 2026-05-27 (**migration `0010` PENDING**). Stock counts + visitors-on-report not started.
...
## Phase 5 (operations — machinery built, migration pending)
**Machinery daily log.** Machines are company-scoped reference data (managed in `/office/catalog`, "Machines" section, pm/office). Supervisor logs a machine per day at `/app/projects/[id]/machinery` (`machine-log-form.tsx`): machine + date + present / hours / breakdown(+note) / operator / fuel (per-day total +note). Upsert keyed `(project_id, machine_id, log_date)` so re-logging updates the day. Office sees a read-only recent-logs list on the project page. Data: `machinery.ts` (`getMachines`/`getProjectMachineLogs`). Actions: `createMachine`/`logMachine`. Migration `0010_machinery.sql` adds `machines` + `machine_logs` + RLS + grants. **Remaining Phase 5: weekly stock counts (consumption derived) + visitors/equipment as secondary daily-report fields.** (Same client/server gotcha as taxonomy: client form imports only `type Machine`, inlines the label — never import a runtime value from a `server-only` module into a client component.)

## Migrations
**Applied & verified through 0008 + storage_setup on hosted Supabase. PENDING apply: `0009` (purchase requests), `0010` (machinery).**
